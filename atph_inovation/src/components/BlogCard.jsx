...
import { potongText, stripHtml } from '../lib/helper'
...
const BlogCard = ({blog}) => {
  ...
  return (
    <div onClick={()=>navigate(`/blog/${id}`)}
        ...
        <img 
          src={import.meta.env.VITE_IMAGE_BLOG + image} 
          alt={title} 
          className='aspect-vidio' />
        ...
        <div className='p-5'>
            ...
            <p className='mb-3 text-xs text-gray-600'>{stripHtml(potongText(description, 100))}</p>
        </div>
    </div>
  )
}
